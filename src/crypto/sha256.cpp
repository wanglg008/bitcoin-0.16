...
//该函数的功能是：自动选择最佳有效的SHA256实现、返回实现的名称。即这个函数选择了最合适的实现SHA256的方式，并在日志中打印该消息。
std::string SHA256AutoDetect()
{
#if defined(USE_ASM) && (defined(__x86_64__) || defined(__amd64__))
    //对于64位的系统通过__get_cpuid获取cpuid，然后通过ecx来获取具体芯片的feature flags，具体可以参
    //考https://en.wikipedia.org/wiki/CPUID，ecx的第19位正好对应sse4.1的指令，其他情况就返回standard。
