...
/**
 * Wrapped mutex: supports recursive locking, but no waiting            封装的boost互斥锁:支持递归锁定，但不支持等待
 * TODO: We should move away from using the recursive lock by default.  TODO:我们应该避免在默认情况下使用递归锁。
 *///类继承AnnotatedMixin类，而且有boost::recursive_mutex的类模板。可以知道CCriticalSection类定义的对象都是boost::recursive_mutex的类，而这是个互斥的类
...
//LOCK实现的是对包含的代码在各线程中进行互斥加锁处理，防止后续代码中涉及的全局变量被不同的线程抢夺。
