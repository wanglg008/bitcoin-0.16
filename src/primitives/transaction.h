...
//系统定义了交易的输出点，包括交易输出的哈希值、交易输出数组索引
...
public:
    ...
    friend bool operator<(const COutPoint& a, const COutPoint& b)   //重载小于号
    {
...
/** An input of a transaction.  It contains the location of the previous 交易的输入，包括当前输入对应前一笔交易的输出的位置，以
 * transaction's output that it claims and a signature that matches the  及花费前一笔输出需要的签名脚本CScriptWitness是用来支
 * output's public key.                                                  持隔离见证时使用的.
 *///系统定义了交易输入、包含上一个交易输出点的位置、公钥的签名。备注：此结构主要说明用于交易的比特币的来源
class CTxIn
{
public:
    ...
    CScriptWitness scriptWitness; //! Only serialized through CTransaction  用来支持隔离见证时使用的
 ...
 *///系统定义交易输出，包含了交易的比特币数量、公钥，以及一些标志。备注：此结构主要表明交易发送时的比特币数量、接收方的发送地址、标志。
class CTxOut
{
public:
 ...
 *///系统定义了交易信息，在网络中传播。交易信息可以包含多个交易输入和交易输出。
class CTransaction
{
public:
