...
bool CKey::Check(const unsigned char *vch) {
    return secp256k1_ec_seckey_verify(secp256k1_context_sign, vch); //通过调用libsecp256k1库实现随机数的验证的：如果返回值如果为1，密钥有效；如果为0则无效。传入的两个参数ctx与seckey均不能为NULL，都需要有值。
}
//在MakeNewKey()函数中是通过GetStrongRandBytes()函数循环获取私钥，直到获取的私钥满足Check()函数验证条件时才停止。
void CKey::MakeNewKey(bool fCompressedIn) {
    do {
        GetStrongRandBytes(keydata.data(), keydata.size());
    } while (!Check(keydata.data()));
    fValid = true;  //获得私钥并检查符合作为私钥的条件后，将私钥有效性标志fValid设置为true，同时将传入的fCompressedIn值赋值给fCompressed，用于标识是否使用压缩公钥。
...
//调用了secp256k1_ec_pubkey_create()函数创建公钥值，然后调用secp256k1_ec_pubkey_serialize()函数实现压缩或非压缩公钥序列值的计算。
...
bool CKey::VerifyPubKey(const CPubKey& pubkey) const {
    ...
    GetRandBytes(rnd, sizeof(rnd)); //通过OpenSSL的GetRandBytes()函数实现随机数的生成；
    uint256 hash;                   //根据"Bitcoin key verification\n"字符串与刚生成的随机数共同作用计算随机哈希值；
    CHash256().Write((unsigned char*)str.data(), str.size()).Write(rnd, sizeof(rnd)).Finalize(hash.begin());
    std::vector<unsigned char> vchSig;
    Sign(hash, vchSig);             //在Sign函数通过该随机哈希值基于ECDSA算法实现签名值的计算；
    return pubkey.Verify(hash, vchSig);//利用该签名信息验证获取的公钥的有效性，验证函数位于CPubKey的Verify()函数中。
}
