...
//LoadGenesisBlock()函数是检测网络类型等后在唯一一个创世区块基础上写入新的区块文件。
bool CChainState::LoadGenesisBlock(const CChainParams& chainparams)
{
    LOCK(cs_main);

    // Check whether we're already initialized by checking for genesis in   通过检查mapBlockIndex中的起源来检查我们是否已经初始化。
    // mapBlockIndex. Note that we can't use chainActive here, since it is  需要注意的是，我们不能在这里使用chainActive，
    // set based on the coins db, not the block index db, which is the only 因为它是基于比特币数据库，而不是在此处加载的唯一的区块索引数据库设置的。
    ...
    try {
        ...
        CDiskBlockPos blockPos = SaveBlockToDisk(block, 0, chainparams, nullptr);//开始新的区块文件
        if (blockPos.IsNull())
            return error("%s: writing genesis block to disk failed", __func__);
        CBlockIndex *pindex = AddToBlockIndex(block);
        CValidationState state;
        //当没有从磁盘上的blk * .dat文件重建链状态和块索引时，也没有在唯一一个创世区块基础上写入新的区块文件，那么会提示：“初始化块数据库时出错”，并跳出循环体。
        if (!ReceivedBlockTransactions(block, state, pindex, blockPos, chainparams.GetConsensus()))
