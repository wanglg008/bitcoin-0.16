...
//创建并返回一个std::unique_ptr<CChainparams>的选择链。返回所选链的CChainParams*指针。如果这个链没有被支持则抛出一个错误信息：std::runtime_error
...
//该函数实现的功能就是把ChainNameFromCommandLine()函数获取得到的网络名称设置成对应的基本的网络基本参数。
void SelectParams(const std::string& network)
{
    SelectBaseParams(network);
    globalChainParams = CreateChainParams(network); //根据得到的网络名称设置成对应的网络参数，并把参数赋值给globalChainParams。
}
